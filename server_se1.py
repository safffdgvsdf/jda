...
class Server:
    ...
    def __init__(self, args, model, dataset, dict_users):
        ...
        self.fftanchorloss = AnchorfftLoss(self.args.num_classes, self.args.dims_feature).to(args.device)
        ...
        for i in range(self.args.K):
            temp2 = copy.deepcopy(self.fftanchorloss)
            clients = [str(i) for i in range(self.args.K)]
            temp2.name = clients[i]
            self.clsf.append(temp2)
            ...
            if fedbn:
                for i in index:
                    ...
                    if self.nns[i] == []:
                        ...
                        a = self.nns[i]
                        client_w = a.state_dict()
                        for key in global_w:
                            if 'bn' not in key:
                                client_w[key] = global_w[key] 
                        a.load_state_dict(client_w)
            else:
    ...
    def fedfa_anchorloss(self, testset, dict_users_test, similarity=True, fedbn = False,
        ...
        for t in range(self.args.r):
            ...
            #joint updating to obtain personalzied model based on updating global model
            self.cls, self.clsf, self.nns, self.loss_dict  = client_fedfa_cl(self.args, index, self.cls, self.clsf, self.nns, self.nn, t, self.dataset,  self.dict_users, self.loss_dict)
            ...
            if similarity:
                ...
                for k in index:
                    classifier_weight_update =  self.nns[k].classifier.weight.data - self.nn.classifier.weight.data
                    classifier_bias_update = self.nns[k].classifier.bias.data.view(15,1) -self.nn.classifier.bias.data.view(15,1)
                    client_classifiers[k] = torch.cat([classifier_weight_update,
                                                       classifier_bias_update],1)
                for p, k in enumerate(index):
                    for q, j in enumerate(index):
        ...
        df1 = pd.DataFrame(mean_CKA_dict)
        df1.to_excel( 'C:\\Users\lenovo\Desktop\mydata\ acc_fedFA_{}_{}_K[{}]_iid[{}]_E[{}]_B[{}].xlsx'.
                format(self.args.dataset, self.args.r, 
                self.args.K, self.args.alph, self.args.E, self.args.B)) 

        # for k in range(self.args.K):df
        #     path="results/Test/{} skew/{}/fea/seed{}/client{}_model_fedfa_{}E_{}class".format(self.args.skew,
        #                                         self.args.dataset, self.args.seed,k, self.args.E, self.args.split)
        #     if self.nns[k]!=[]:
        #         torch.save(self.nns[k].state_dict(), path)
        self.nns = [[] for i in range(self.args.K)]
    ...
    def fedbn(self, testsets, dict_test, clients_dataset_index, similarity=True,
              fe_optimizer_name = "fedavg",test_global_model_accuracy = True):
