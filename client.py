...
def client_fedfa_cl(args, client_index, anchorloss_funcs,fftanchorloss_funcs, client_models, global_model, global_round, dataset_train, dict_users, loss_dict):  # update nn
    for k in client_index: #k is the index of the client
        if args.verbose:
            print('client {} client_fedfa_anchorloss...'.format(k))
        anchorloss_funcs[k],fftanchorloss_funcs[k],client_models[k], loss = op.fedfa_cl_optimizer(args, anchorloss_funcs[k],fftanchorloss_funcs[k], client_models[k], global_model, global_round, dataset_train, dict_users[k])
        loss_dict[k].extend(loss)
    ...
    return anchorloss_funcs, fftanchorloss_funcs,client_models, loss_dict
