...
def sample_gt(gt, train_size, mode='random'):
    ...
    if mode == 'random':
       ...
       # Convert lists of lists to tuple of numpy arrays for indexing
       train_indices = tuple(map(np.array, train_indices))
       test_indices = tuple(map(np.array, test_indices))

       # Validate index range
       if train_indices[0].max() >= gt.shape[0] or train_indices[1].max() >= gt.shape[1]:
           raise IndexError(f"Train index out of bounds: {train_indices}")

       if test_indices[0].max() >= gt.shape[0] or test_indices[1].max() >= gt.shape[1]:
           raise IndexError(f"Test index out of bounds: {test_indices}")
